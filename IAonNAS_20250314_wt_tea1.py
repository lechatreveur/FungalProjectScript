...
# -*- coding: utf-8 -*-
"""
Created on Wed Apr  2 16:16:39 2025
...
# Paths
input_ims = "/Volumes/Movies/20250314/20250314_wt tea1/20250314_wt tea1.ims"
output_frames_folder = "/Volumes/Movies/20250314/20250314_wt tea1/Frames"
output_masks_folder = "/Volumes/Movies/20250314/20250314_wt tea1/Masks"
custom_model_path = "/Volumes/Movies/20250314/20250314_wt tea1/Frames/models/CP_20250321_180112"
...
with tqdm(total=total_tasks, desc="Processing time points") as pbar:
    for t in range(time_points):
        ...
        for c in [0, 1]:
            ...
            projected_filename = f"20250314_wt tea1_t_{t:02d}_c_{c:01d}.tif"
            ...
            seg_filename = f"20250314_wt tea1_t_{t:02d}_c_{c:01d}_seg.tif"
        ...
        img_name = f"20250314_wt tea1_t_{t:02d}_z_{z:01d}_c_{c:01d}.tif"
        ...
        masks, flows, styles, *extra = model.eval(selected_img,
                                                  channels=channels_for_segmentation,
                                                  diameter=diameter)
        seg_filename = f"20250314_wt tea1_t_{t:02d}_z_{z:01d}_c_{c:01d}_seg.tif"
...
out_dir = "/Volumes/Movies/20250314/20250314_wt tea1/TrackedCells"
...
def create_movie(channel, z_index=None, frame_dir=output_frames_folder, output_dir=out_dir, fps=10):
    if channel in [0, 1]:
        frame_pattern = f"20250314_wt tea1_t_??_c_{channel}.tif"
        movie_name = f"20250314_wt tea1_c_{channel:01d}.mp4"
    elif channel == 2 and z_index is not None:
        frame_pattern = f"20250314_wt tea1_t_??_z_{z_index}_c_{channel}.tif"
        movie_name = f"20250314_wt tea1_z_{z_index:01d}_c_{channel:01d}.mp4"
    else:
...
output_tracked_cells_folder = "/Volumes/Movies/20250314/20250314_wt tea1/TrackedCells"
...
for cell in filtered_regions:
    ...
    for t, bf_mask_path in enumerate(mask_paths):
        ...
        c = 0 
        fluorescent_GFP_frame_path = os.path.join(output_frames_folder, f"20250314_wt tea1_t_{t:02d}_c_{c:01d}.tif")
        c = 1
        fluorescent_RFP_frame_path = os.path.join(output_frames_folder, f"20250314_wt tea1_t_{t:02d}_c_{c:01d}.tif")
        c = 2
        z = z_index
        brightfield_frame_path = os.path.join(output_frames_folder, f"20250314_wt tea1_t_{t:02d}_z_{z:01d}_c_{c:01d}.tif")
        ...
        # c = 0
        # gfp_seg_path = os.path.join(GFP_seg_folder, f"20250314_wt tea1_1_F0-007_t_{t:02d}_c_{c:01d}_seg.tif")
        ...
        # c = 1
        # rfp_seg_path = os.path.join(RFP_seg_folder, f"20250314_wt tea1_1_F0-007_t_{t:02d}_c_{c:01d}_seg.tif")
