...
def process_ims_file(ims_path):
    ...
    with tqdm(total=time_points * 2, desc=f"Segmenting {file_name}") as pbar:
        for t in range(time_points):
            ...
            frame_path = os.path.join(output_frames_folder, f"{file_name}_t_{t:02d}_c_0.tif")
            imwrite(frame_path, projected_img)
            masks, *_ = model.eval(projected_img, channels=channels_for_segmentation, diameter=diameter)
            seg_path = os.path.join(GFP_seg_folder, f"{file_name}_t_{t:02d}_c_0_seg.tif")
            ...
            frame_path = os.path.join(output_frames_folder, f"{file_name}_t_{t:02d}_z_{z:01d}_c_1.tif")
            imwrite(frame_path, selected_img)
            masks, *_ = model.eval(selected_img, channels=channels_for_segmentation, diameter=diameter)
            seg_path = os.path.join(brightfield_seg_folder, f"{file_name}_t_{t:02d}_z_{z:01d}_c_1_seg.tif")
            imwrite(seg_path, masks)
