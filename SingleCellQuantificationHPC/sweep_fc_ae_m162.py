#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Latent-dimension sweep for the M162 autoencoder — find the elbow.
...
def load(features_dir, keep_division, film_contains=None):
    X_traj, X_feat, gids, labels, s_traj, s_feat = load_feature_constrained_data(
        {EXP_KEY: str(features_dir)})
    if not keep_division:
        feats = pd.read_csv(Path(features_dir) / f"umap_features_{EXP_TAG}.csv")
...
def main():
    ...
    try:
        ...
        ax.set_title(f"M162 MultimodalAutoencoder3D latent sweep "
    ...
    (a.out / "_provenance.json").write_text(json.dumps(dict(
        ...
        created_by="SingleCellQuantificationHPC/sweep_fc_ae_m162.py",
