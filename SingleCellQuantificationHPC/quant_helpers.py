...
# -------------------------------------------------------------------------
# Vertical-strip tiles (built directly from the quantification pipeline)
# -------------------------------------------------------------------------
def build_strip_tile(crop_img, crop_mask, frame_h=32, frame_w=112, pad_rot=100):
    """
    Build one fixed-size (frame_h x frame_w) strip tile from an ALREADY-TIGHT
    per-frame crop (crop_img: 2D array; crop_mask: boolean array, same shape).

    This reuses the geometry of the legacy generate_M156_strips.py vertical
    strip generator (rotate cell to horizontal, tight-crop, scale to fit,
    center on a black frame_h x frame_w canvas) but is fed the FRESH,
    un-rotated tight bbox crop that ImageQuantification()/bf_pattern_only()
    already compute every frame -- not a single reference-frame rotation
    angle applied to the whole trajectory, which is what made the old strips
    fragile.

    Rotation convention: skimage's regionprops.orientation is measured from
    the ROW (vertical) axis, not from horizontal, so the angle that
    straightens a cell to horizontal is -(angle_deg - 90), NOT -angle_deg.
    Using -angle_deg directly over-rotates by ~90 degrees for most cells and
    was the root cause of the "blunt poles / blobby masks" bug fixed
    2026-08-24 (see project memory: m156_strip_rotation_bug_and_id_drift).

    Returns a float32 (frame_h, frame_w) array in the caller's input
    intensity units (an all-zero tile if the mask is empty/degenerate). The
    caller is responsible for any cross-frame normalization + colormapping
    (see save_strip_from_tiles) once all of a cell's tiles are collected.
    """
    crop_img = np.asarray(crop_img, dtype=np.float32)
    crop_mask = np.asarray(crop_mask, dtype=bool)
    if crop_img.shape != crop_mask.shape or not crop_mask.any():
    ...
    # Pad generously before rotating so the ends don't clip.
    ...
    if rows_r.size == 0 or cols_r.size == 0:
        return np.zeros((frame_h, frame_w), dtype=np.float32)
    ...
    # Ensure wide > tall (long axis = width).
    ...
    scale = frame_w / 210.0
    new_h = max(1, min(frame_h, int(tight_img.shape[0] * scale)))
    new_w = max(1, min(frame_w, int(tight_img.shape[1] * scale)))
    resized = cv2.resize(tight_img, (new_w, new_h), interpolation=cv2.INTER_LINEAR)

    canvas = np.zeros((frame_h, frame_w), dtype=np.float32)
    start_h = (frame_h - new_h) // 2
    start_w = (frame_w - new_w) // 2
    canvas[start_h:start_h + new_h, start_w:start_w + new_w] = resized
    return canvas


def save_strip_from_tiles(tiles, out_path, cmap_name='viridis'):
    """
    Stack a list of build_strip_tile() outputs (temporal order, one per
    frame actually quantified -- gaps are simply not present, not padded)
    into one vertical strip PNG, using the same per-strip 99th-percentile
    normalization + viridis colormap as the legacy strip generator so the
    output is visually comparable to what reviewers already know.

    No-op (returns False) if `tiles` is empty.
    """
    if not tiles:
        return False
    strip = np.vstack(tiles)  # (n_frames * frame_h, frame_w)

    nonzero = strip[strip > 0]
    p99 = float(np.percentile(nonzero, 99)) if nonzero.size > 0 else 1.0
    if p99 < 1e-5:
        p99 = 1.0

    strip_norm = np.clip(strip / p99, 0, 1) * 255.0
    ...
    if out_dir:
        os.makedirs(out_dir, exist_ok=True)
    Image.fromarray(strip_rgb).save(out_path)
    return True
...
def quantify_one_object(
    ...
    *,
    ...
    extra_cols=None,
    strip_tiles=None,    # Optional list; if given, one build_strip_tile() output
                          # is appended per top-level (parent) call. Independent
                          # of do_plot -- strips are a cheap, separate artifact.
):
    """
    ...
    # Strip tile: only for the top-level (parent) object, not split children
    # (id_suffix != '' means this is a recursive '_1'/'_2' child call), so a
    # cell's strip follows its primary track, not its split-hypothesis tracks.
    if strip_tiles is not None and id_suffix == '':
        strip_tiles.append(build_strip_tile(cropped_img, cropped_cell_mask))
