...
def atomic_write_text(path: Path, content: str) -> None:
    """Atomically write string content to target path using a temporary file."""
    ...
    try:
        ...
        # Fallback to direct write if atomic replace/fsync fails (e.g. on exFAT)
...
class MaskRepository:
    ...
    def load_cell_masks(self, exp: str, film: str, cell_id: int) -> Tuple[pd.DataFrame, str]:
        ...
        # Ensure DataFrame is sorted by time_point
    ...
    def save_cell_masks(self, exp: str, film: str, cell_id: int, df: pd.DataFrame, expected_revision: Optional[str] = None) -> str:
        ...
        # Validate time_point identity
        if "time_point" not in df.columns:
            raise DataIntegrityError("DataFrame missing required 'time_point' column")
