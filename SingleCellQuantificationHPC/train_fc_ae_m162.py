#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Train an M162 multimodal autoencoder (stage 6 input).
...
Training a separate model on M162 gives a genuinely **standalone** manifold;
projecting M162 through the Sept17 model would instead put it on the reference
...
The loader is pointed at the M162 features directory, which carries the layout
...
`M162_<global_cell_id>_<film>`, matching the reference's experiment + cell +
source convention.
...
DEFAULT_MODEL = _SSD_OUT / f"fc_ae_3d_{EXP_TAG}.pth"
...
SEED = 42
# Chosen by sweep_fc_ae_m160.py on 6,184 M162 datapoints: the elbow sits at
...
def main():
    ...
    print("loading M162 trajectories and features ...", flush=True)
    ...
    if not a.keep_division_films:
        feats = pd.read_csv(Path(a.features_dir) / f"umap_features_{EXP_TAG}.csv")
        if "is_division_film" in feats.columns:
            drop = {f"{EXP_KEY}_{r.global_cell_id}_{r.film}"
    ...
    prov = dict(
        ...
        created_by="SingleCellQuantificationHPC/train_fc_ae_m162.py",
        experiment=EXP_NAME, standalone=True,
        note="trained on M162 only; latents are not comparable with the Sept17 model",
