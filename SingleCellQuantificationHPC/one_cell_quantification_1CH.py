...
def track_one_direction(t_seq, ref_start_mask, channel='bf',
    ...
    prev_meta = {'composition': 'single'}  # start as single unless you have starter metadata
    ...
    for i, t in enumerate(t_seq):
        ...
        if i == 0 and lock_first:
            ...
            xinfo = {}
            composition = prev_meta.get('composition', 'single')
            segments_rle = prev_meta.get('segments_rle', None)
        else:
            # ======= AREA-AWARE SELECTOR with pair-aware mode =======
            area_selected = False
            area_meta_out = None

            if lab_cur is not None and xcorr_mode != 'primary':
                # If previous selection was a pair, track each segment as a SINGLE.
                if prev_meta.get('composition') == 'pair' and prev_meta.get('segments_rle'):
                    # reconstruct previous pair segments
                    seg_masks_prev = [rle_to_mask(rle) for rle in prev_meta['segments_rle']]
                    # safety: build union ref to compute combined overlap later
                    ref_union = seg_masks_prev[0] | seg_masks_prev[1]

                    per_seg = []
                    for seg_ref in seg_masks_prev:
                        cm_s, ov_s, sc_s, pen_s, rej_s, meta_s = get_cell_mask_area_aware(
                            lab_cur, seg_ref, prev_area,  # prev_area is from UNION; OK to reuse here
                            threshold=thr, max_segments=1, topk=topk,
                            area_lambda=area_lambda, ratio_soft=ratio_soft, ratio_hard=ratio_hard
                        )
                        per_seg.append((cm_s, ov_s, sc_s, pen_s, rej_s, meta_s))

                    # union the two picked singles
                    cm0 = per_seg[0][0] | per_seg[1][0]
                    # recompute combined metrics against previous UNION
                    ov0  = compute_overlap(ref_union, cm0)
                    a0   = int(cm0.sum())
                    pen0 = area_change_penalty(a0, prev_area, ratio_soft, ratio_hard)
                    sc0  = ov0 - area_lambda * pen0
                    rej0 = bool(per_seg[0][4] or per_seg[1][4])
                    area_selected = True
                    sel_mode_area = 'area_pair_track'
                    # record NEW per-segment RLEs
                    area_meta_out = {
                        'composition': 'pair',
                        'segments_rle': [mask_to_rle(per_seg[0][0]), mask_to_rle(per_seg[1][0])]
                    }
                else:
                    # Normal (single→single or single→pair allowed)
            ...
            if use_xcorr and lab_cur is not None:
                ...
                if xsel["best"] is not None:
                    ...
                    # Optional: infer composition from labels if available
                    if isinstance(xsel["best"]["labels"], (list, tuple)) and len(xsel["best"]["labels"]) == 2:
                        # store per-segment RLEs for those labels
                        la, lb = xsel["best"]["labels"]
                        cm_a = (lab_cur == la)
                        cm_b = (lab_cur == lb)
                        xcorr_meta = {
                            'composition': 'pair',
                            'segments_rle': [mask_to_rle(cm_a), mask_to_rle(cm_b)]
                        }
                    else:
            ...
            segments_rle = meta_out.get('segments_rle', None)
        ...
        results[t] = {
            ...
            **({"segments_rle": segments_rle} if composition == 'pair' and segments_rle else {}),
            **({} if not xinfo else xinfo),
        }
        prev_mask = cm
        prev_area = float(cm.sum())
        prev_meta = {"composition": composition}
        if composition == 'pair' and segments_rle:
            prev_meta['segments_rle'] = segments_rle
...
if __name__ == "__main__":
    ...
    if need_to_track:
        ...
        # right before the for t in range(frame_number): loop, keep H,W as is
        ...
        for t in range(frame_number):
            ...
            if comp == "pair" and ch_sel.get("segments_rle"):
                # segments_rle are dict-RLEs; convert to string-RLEs for the CSV
                seg_masks = [rle_to_mask(ch_sel["segments_rle"][0]),
                             rle_to_mask(ch_sel["segments_rle"][1])]
                pair_a_str = rle_encode(seg_masks[0])
                pair_b_str = rle_encode(seg_masks[1])
