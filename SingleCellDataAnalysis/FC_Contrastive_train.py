...
class FCTrajectoryContrastiveDataset(Dataset):
    def __init__(self, X_traj, X_feat, transition_path):
        ...
        self.transitions = np.load(transition_path) # (M, 2) [parent_idx, daughter_idx]
    ...
    def __getitem__(self, idx):
        ...
        # View 2: Self-Augment or Lineage Partner
        if np.random.rand() > 0.8: # 20% lineage stitching
            matches = self.transitions[self.transitions[:, 0] == idx]
            if len(matches) > 0:
                partner_idx = int(matches[np.random.randint(len(matches))][1])
                t2 = self.augment_traj(self.X_traj[partner_idx])
                f2 = self.augment_feat(self.X_feat[partner_idx])
            else:
...
def main():
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if epoch % 50 == 0 or epoch == 1:
            ckpt_path = os.path.join(BASE_DIR, f"checkpoints/fc_contrastive_epoch_{epoch:03d}.pth")
            torch.save(model.state_dict(), ckpt_path)
            print(f"Saved: {ckpt_path}")

    final_path = os.path.join(BASE_DIR, "fc_contrastive_final.pth")
