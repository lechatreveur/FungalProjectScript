...
OUTPUT_HTML = "/Volumes/X10 Pro/FungalProject_Outputs/fc_contrastive/fc_manifold_explorer_no_lineage_M133.html"
...
def main():
    ...
    with torch.no_grad():
        ...
        latents_M133 = model(torch.from_numpy(X_traj_M133).float(), torch.from_numpy(X_feat_M133).float()).numpy()
    ...
    fig_3d = px.scatter_3d(df_3d, x="x", y="y", z="z", color="color", symbol="exp", hover_data=["gid"],
                           title="Fungal Manifold (No Lineage) + M133", color_continuous_scale="Viridis")
...
<head>
  <meta charset="utf-8"/>
  <title>Contrastive Explorer 2D/3D + M133</title>
  <style>
