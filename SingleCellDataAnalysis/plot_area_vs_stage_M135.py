...
MOVIE_ROOT = Path("/Volumes/X10 Pro/Movies/2026_04_30_M135")
...
def main():
    ...
    # We use F0, which is the curated FOV
    fov = "A14_F0"
    if fov not in linkage:
        print("Error: A14_F0 not found in linkage.")
        return
        
    FILM_SEQUENCE = linkage[fov]["films"]
    global_cells = linkage[fov]["global_cells"]
    
    # Load curated status
    qc_path = MOVIE_ROOT / "A14_F0" / "qc_A14_F0.json"
    ...
    df_plot.to_csv(ARTIFACTS_DIR / "global_area_vs_stage_data_M135.csv", index=False)
    ...
    plt.title("Continuous Fission Yeast Cell Area vs Time Relative to Division (M135)", fontsize=12, fontweight='bold')
    plt.xlabel("Time relative to division (minutes)")
    plt.ylabel("Cell Area (px²)")
    plt.grid(True, linestyle='--', alpha=0.5)
    plt.legend()
    
    out_img = ARTIFACTS_DIR / "cell_area_vs_stage_M135.png"
