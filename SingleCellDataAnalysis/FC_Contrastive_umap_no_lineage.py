...
# -*- coding: utf-8 -*-
"""
FC_Contrastive_umap_no_lineage.py
Self-contained 3D and 2D UMAP explorer with:
...
def load_cell_areas(gids):
    ...
    for i, gid in enumerate(gids):
        ...
        if film == '':
            csv_p = os.path.join(base, f"TrackedCells_A14_10_20min", f"cell_{orig_id}_data.csv")
        elif film is None:
            continue
        else:
...
def main():
    ...
    model.eval()
    with torch.no_grad():
        latents = model(torch.from_numpy(X_traj).float(), torch.from_numpy(X_feat).float()).numpy()
        
    embedding_3d = umap.UMAP(n_components=3, random_state=42, n_jobs=1).fit_transform(latents)
    embedding_2d = umap.UMAP(n_components=2, random_state=42, n_jobs=1).fit_transform(latents)
    ...
    fig_3d = px.scatter_3d(df_3d, x="x", y="y", z="z", color="color", symbol="exp", hover_data=["gid"],
                           title="Fungal Multimodal Manifold Explorer (No Lineage Stitching)", color_continuous_scale="Viridis")
...
<head>
  <meta charset="utf-8"/>
  <title>Fungal Manifold Explorer 2D/3D (No Lineage)</title>
  <style>
...
<body>
  ...
  <script>
    ...
    var is3D = true;
    var currentPlot = JSON.parse(JSON.stringify(plotData3D));
    ...
    function renderPlot() {
        ...
        currentPlot = JSON.parse(JSON.stringify(basePlot));
