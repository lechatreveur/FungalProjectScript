...
LOG_FILE     = os.path.join(OUTPUT_DIR, "train_physics.log")
    ...
    with open(LOG_FILE, 'w') as log_f:
        ...
        for epoch in range(1, EPOCHS + 1):
            ...
            for v_batch, t_batch, f_batch in dataloader:
                ...
                optimizer.zero_grad()
                v_hat, t_hat, f_hat, _, m1, m2 = model(v_batch)
            ...
            if epoch % SAVE_EVERY == 0 or epoch == EPOCHS:
                ckpt = os.path.join(OUTPUT_DIR, f"video_physics_epoch_{epoch:03d}.pth")
                torch.save(model.state_dict(), ckpt)
                log_print(f"  💾 Checkpoint: {ckpt}")

        # 6. Save final model
        final = os.path.join(OUTPUT_DIR, "video_physics_final.pth")
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(OUTPUT_DIR, "physics_training_loss.png"), dpi=150)
