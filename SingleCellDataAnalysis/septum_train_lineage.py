...
def load_lineage_observations(
    ...
    for segment in lineage["segments"]:
        with np.load(segment["npz_path"], allow_pickle=True) as sample:
...
def build_targets(
    ...
    for event in lineage["events"]:
        ...
        if start is not None and end is not None:
            state[(times >= float(start)) & (times <= float(end))] = 1.0
        elif start is None and end is not None:
            # The unobserved start makes pre-end state ambiguous.
            state_known[times <= float(end)] = 0.0
            has_left_censoring = True
        elif start is not None and end is None:
            # The unobserved end makes post-start state ambiguous.
            state_known[times >= float(start)] = 0.0
...
class LineageWindowDataset(Dataset):
    ...
    def __getitem__(self, index: int) -> dict[str, Any]:
        ...
        targets = build_targets(lineage, selected_times, self.endpoint_sigma_min)
...
def main() -> None:
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        torch.save(model.state_dict(), args.output_dir / "model_latest.pt")
        (args.output_dir / "history.json").write_text(
            json.dumps(history, indent=2), encoding="utf-8"
        )

    model.load_state_dict(
        torch.load(args.output_dir / "model_best.pt", map_location=device, weights_only=True)
    )
