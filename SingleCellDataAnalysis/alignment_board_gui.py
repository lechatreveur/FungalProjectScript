...
def review_septum_alignment_board_gui(
    ...
    offsets, global_interval, last_row_start, last_a_left, cell_order, cell_intervals = load_state(
    ...
    ax_btn_export = fig.add_axes([0.90, 0.04, 0.08, 0.06])
    ...
    # global or cell intervals
    label_mode = "global"  # "global" or "cell"
    mode_text = fig.text(0.55, 0.02, "Mode: GLOBAL (g/l)", fontsize=10)
    
    cell_intervals = cell_intervals if isinstance(cell_intervals, dict) else {}
    ...
    def _get_active_interval():
        # returns (start, end) in aligned coords, or None
        if label_mode == "cell" and active_cell_id is not None:
            v = cell_intervals.get(int(active_cell_id), None)
            if v and v.get("has_septum", False):
                s = v.get("start_aligned", None)
                e = v.get("end_aligned", None)
                if s is not None and e is not None:
                    return (int(round(float(s))), int(round(float(e))))
    ...
    def _update_interval_artist():
        ...
        interval = _get_active_interval()
        if interval is not None:
            G0, G1 = interval
            interval_span = ax_time.axvspan(G0, G1, alpha=0.2)
            if label_mode == "cell":
                interval_text.set_text(f"Cell interval: [{G0}, {G1}]")
            else:
                interval_text.set_text(f"Global interval: [{G0}, {G1}] (inclusive)")
        else:
            if label_mode == "cell":
                interval_text.set_text("Cell interval: (not set) — drag to set")
            else:
    ...
    def _on_span(xmin, xmax):
        nonlocal global_interval, cell_intervals
        ...
        if label_mode == "global":
            ...
            if active_cell_id is None:
                return
            cell_intervals[int(active_cell_id)] = {
                "has_septum": True,
                "start_aligned": float(G0),
                "end_aligned": float(G1),
            }
    ...
    def _clear_interval():
        ...
        if label_mode == "global":
            ...
            if active_cell_id is not None and int(active_cell_id) in cell_intervals:
                cell_intervals.pop(int(active_cell_id), None)
        _update_interval_artist()
        fig.canvas.draw_idle()
    ...
    # prediction    
    pred_span = None
    pred_text = ax_time.text(0.65, 0.6, "", transform=ax_time.transAxes)
    
    def predict_for_cell_stub(cid: int):
        # TODO: replace with real model call
        return None  # or (start, end, score)
    ...
    def _update_prediction_overlay():
        ...
        pred = predict_for_cell_stub(int(active_cell_id))
        if pred is None:
            pred_text.set_text("Pred: (none)")
            return
    
        s, e, score = pred
        pred_span = ax_time.axvspan(int(s), int(e), alpha=0.12)
        pred_text.set_text(f"Pred: [{int(s)}, {int(e)}] score={score:.3f}")
    ...
    btn_export.on_clicked(_on_export)
    ...
    def _on_key(evt):
        ...
        if k == ",":
            ...
            _render_light()
        # elif k == "e":
        #     # export selected if any, else active
        #     if selected_cell_ids:
        #         _export_training_for_cells(sorted(selected_cell_ids))
        #     elif active_cell_id is not None:
        #         _export_training_for_cells([active_cell_id])
        elif k == "e":
