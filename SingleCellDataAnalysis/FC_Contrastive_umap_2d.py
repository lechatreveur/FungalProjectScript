...
# -*- coding: utf-8 -*-
"""
FC_Contrastive_umap_2d.py
Self-contained 2D UMAP explorer with:
...
OUTPUT_HTML = "/Volumes/X10 Pro/FungalProject_Outputs/fc_contrastive/fc_manifold_explorer_2d.html"
...
def load_cycle_scores(gids):
    ...
    print(f"🔗 Cycle scores matched: {np.sum(~np.isnan(scores))}/{len(gids)} cells")
    return scores
...
def load_cell_areas(gids):
    """Return max(cell_area) per cell from cell_data.csv via id_map resolution."""
    ...
    for i, gid in enumerate(gids):
        ...
        if film == '':
            ...
            csv_p = os.path.join(base, film, f"TrackedCells_{film}", f"cell_{orig_id}_data.csv")
...
def remap_for_display(score: float) -> float:
    ...
    Piecewise-linear remap so Stage 1 (raw 0–0.4) fills 80% of the colorscale.
      Raw 0.0–0.4 (Stage 1) → Display 0.00–0.80
      Raw 0.4–0.7 (Stage 2) → Display 0.80–0.90
      Raw 0.7–0.9 (Stage 3) → Display 0.90–0.95
      Raw 0.9–1.0 (Stage 4) → Display 0.95–1.00
    """
...
def main():
    ...
    print("🗺️ Running 2D UMAP...")
    embedding = umap.UMAP(n_components=2, random_state=42, n_jobs=1).fit_transform(latents)
    ...
    fig = px.scatter(df, x="x", y="y",
        ...
        title="Fungal Multimodal Manifold Explorer (2D)",
    ...
    for i, gid in enumerate(gids):
        ...
        traj_dict[gid] = {
            ...
            "f":     [round(float(X_feat[i,1]),4), round(float(X_feat[i,7]),4),
                      round(float(X_feat[i,6]),4),
                      round(float(cycle_scores[i]),4) if not np.isnan(cycle_scores[i]) else None],
...
<head>
  <meta charset="utf-8"/>
  <title>Fungal Manifold Explorer 2D</title>
  <style>
...
<body>
  ...
  <div id="sidebar">
    <div id="content">
      <div class="placeholder">Click any point in the 2D manifold to view its trajectory and features.</div>
    </div>
  </div>
  ...
  <script>
    ...
    with open(OUTPUT_HTML, "w", encoding="utf-8") as f:
        f.write(html)
    print(f"✅ 2D Dashboard saved: {OUTPUT_HTML}")
