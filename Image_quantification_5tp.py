...
# -*- coding: utf-8 -*-
"""
Created on Fri Mar 14 09:50:24 2025
...
output_frame_filename = f"/Users/user/Documents/FungalProject/TimeLapse/Tilescan2/Frames/brightfield_{t:03d}_{z:03d}.tif"
output_mask_filename  = f"/Users/user/Documents/FungalProject/TimeLapse/Tilescan2/Masks/segmented_{t:03d}_{z:03d}.tif"
...
# Process only the first 5 frames (adjust slicing as needed)
for i, frame in enumerate(fluorescent_C1_frames[:5]):
    ...
    fluorescent_C1_filename = os.path.join(output_frames_folder, f"fluorescent_C1_{(i+1):03d}.tif")
...
data = {
    "Y=2_2": [[1, 4, 82], [2, 5, 75], [3, 6, 61], [4, 6, 72], [5, 6, 67]],
    "Y=0":     [[1, 4, 117], [2, 5, 111], [3, 6, 94], [4, 6, 107], [5, 6, 108]],
    "Y=1":     [[1, 4, 106], [2, 5, 99], [3, 6, 82], [4, 6, 99], [5, 6, 93]],
    "Y=2_1": [[1, 4, 115], [2, 5, 110], [3, 6, 93], [4, 6, 110], [5, 6, 103]]
}
ty = "Y=2_1"
tp = 5
cell_ID = data[ty][tp-1][2] -1 
t = data[ty][tp-1][0]
z = data[ty][tp-1][1]
# Load the segmentation mask saved from your script
mask = imread(f"/Users/user/Documents/FungalProject/TimeLapse/Tilescan2/Masks/segmented_{t:03d}_{z:03d}.tif")
...
# File paths
fluorescent_image_file = f"/Users/user/Documents/FungalProject/TimeLapse/Tilescan2/Frames/fluorescent_C1_{t:03d}.tif"
segmentation_mask_file = f"/Users/user/Documents/FungalProject/TimeLapse/Tilescan2/Masks/segmented_{t:03d}_{z:03d}.tif"
...
pi = np.array([0.4999, 0.5, 0.0005, 0.0005])
...
params_linked = {
    'mu_bg_Y0': mu_bg,
    'sigma_bg_Y0': sigma_bg,
    'mu_xy_Y0': mu_xy,
    'sigma_xy_Y0': sigma_xy,
    'mu_I_Y0': mu_bg,
    'sigma_I_Y0': sigma_bg,
    'mu_P_Y0': mu_bg,
    'sigma_P_Y0': sigma_bg,
    'sigma_u1_Y0': sigma_u1,
    'sigma_v1_Y0': sigma_v1,
    'sigma_u2_Y0': sigma_u2,
    'sigma_v2_Y0': sigma_v2,
    
    'mu_bg_Y1': mu_bg,
    'sigma_bg_Y1': sigma_bg,
    'mu_xy_Y1': mu_xy,
    'sigma_xy_Y1': sigma_xy,
    'mu_I_Y1': mu_I,
    'sigma_I_Y1': sigma_I,
    'mu_P_Y1': mu_I,
    'sigma_P_Y1': sigma_I,
    'sigma_u1_Y1': sigma_u1,
    'sigma_v1_Y1': sigma_v1,
    'sigma_u2_Y1': sigma_u2,
    'sigma_v2_Y1': sigma_v2,
...
def state_model(X_data, A, s_par, pi):
    ...
    fZ2 = pdf_polarity_uv(X_data, u_map, v_map, s_par['u_endpoint1'],s_par['sigma_u1_Z2'], s_par['sigma_v1_Z2'], s_par['mu_P_Z2_Z3'], s_par['sigma_P_Z2_Z3'], s_par['L'])
    fZ3 = pdf_polarity_uv(X_data, u_map, v_map, s_par['u_endpoint2'],s_par['sigma_u2_Z3'], s_par['sigma_v2_Z3'], s_par['mu_P_Z2_Z3'], s_par['sigma_P_Z2_Z3'], s_par['L'])
    
    f_total = pi[0]*fZ0 + pi[1]*fZ1 + pi[2]*fZ2 + pi[3]*fZ3
    ...
    return gammaZ0, gammaZ1, gammaZ2, gammaZ3

def E_step(X_data, A, params, Yi, pi):
    s_par_Y0 = {
        'mu_bg_Z0': params['mu_bg_Y0'],
        'sigma_bg_Z0': params['sigma_bg_Y0'],
        'mu_xy_Z1': params['mu_xy_Y0'],
        'sigma_xy_Z1': params['sigma_xy_Y0'],
        'mu_I_Z1': params['mu_I_Y0'],
        'sigma_I_Z1': params['sigma_I_Y0'],
        'mu_P_Z2_Z3': params['mu_P_Y0'],
        'sigma_P_Z2_Z3': params['sigma_P_Y0'],
        'sigma_u1_Z2': params['sigma_u1_Y0'],
        'sigma_v1_Z2': params['sigma_v1_Y0'],
        'sigma_u2_Z3': params['sigma_u2_Y0'],
        'sigma_v2_Z3': params['sigma_v2_Y0'],
        
        'u_map': params['u_map'],
        'v_map': params['v_map'],
        'u_endpoint1': params['u_endpoint1'],
        'u_endpoint2': params['u_endpoint2'],
        'L': params['L']
        }
    gammaY0Z0, gammaY0Z1, gammaY0Z2, gammaY0Z3 = state_model(X_data, A, s_par_Y0, pi) 
    
    s_par_Y1 = {
        'mu_bg_Z0': params['mu_bg_Y1'],
        'sigma_bg_Z0': params['sigma_bg_Y1'],
        'mu_xy_Z1': params['mu_xy_Y1'],
        'sigma_xy_Z1': params['sigma_xy_Y1'],
        'mu_I_Z1': params['mu_I_Y1'],
        'sigma_I_Z1': params['sigma_I_Y1'],
        'mu_P_Z2_Z3': params['mu_P_Y1'],
        'sigma_P_Z2_Z3': params['sigma_P_Y1'],
        'sigma_u1_Z2': params['sigma_u1_Y1'],
        'sigma_v1_Z2': params['sigma_v1_Y1'],
        'sigma_u2_Z3': params['sigma_u2_Y1'],
        'sigma_v2_Z3': params['sigma_v2_Y1'],
    ...
    gammaY1Z0, gammaY1Z1, gammaY1Z2, gammaY1Z3 = state_model(X_data, A, s_par_Y1, pi)     
    ...
    s_par_Y2 = {
        ...
        'mu_P_Z2_Z3': params['mu_P_Y2'],
        'sigma_P_Z2_Z3': params['sigma_P_Y2'],
        'sigma_u1_Z2': params['sigma_u1_Y2'],
        'sigma_v1_Z2': params['sigma_v1_Y2'],
        'sigma_u2_Z3': params['sigma_u2_Y2'],
        'sigma_v2_Z3': params['sigma_v2_Y2'],
    ...
    gammaY2Z0, gammaY2Z1, gammaY2Z2, gammaY2Z3 = state_model(X_data, A, s_par_Y2, pi)     
...
def M_step_linked(X_data, gammas, Yi, pi):
    gammaY0Z0 = gammas['Y0Z0']
    gammaY0Z1 = gammas['Y0Z1']
    gammaY0Z2 = gammas['Y0Z2']
    gammaY0Z3 = gammas['Y0Z3']
    
    gammaY1Z0 = gammas['Y1Z0']
    gammaY1Z1 = gammas['Y1Z1']
    gammaY1Z2 = gammas['Y1Z2']
    gammaY1Z3 = gammas['Y1Z3']
    ...
    mu_xy_new = np.sum(effective_gammaZ1_total[:, np.newaxis] * X_data[:, :2], axis=0) / sum_gammaZ1
    var_x = np.sum(effective_gammaZ1_total * (X_data[:,0]-mu_xy_new[0])**2) / sum_gammaZ1
    var_y = np.sum(effective_gammaZ1_total * (X_data[:,1]-mu_xy_new[1])**2) / sum_gammaZ1
    ...
    params_new = {
        'mu_bg_Y0': mu_bg_new,
        'sigma_bg_Y0': sigma_bg_new,
        'mu_xy_Y0': mu_xy_new,
        'sigma_xy_Y0': sigma_xy_new,
        'mu_I_Y0': mu_bg_new,
        'sigma_I_Y0': sigma_bg_new,
        'mu_P_Y0': mu_bg_new,
        'sigma_P_Y0': sigma_bg_new,
        'sigma_u1_Y0': sigma_u1_new,
        'sigma_v1_Y0': sigma_v1_new,
        'sigma_u2_Y0': sigma_u2_new,
        'sigma_v2_Y0': sigma_v2_new,
        
        'mu_bg_Y1': mu_bg_new,
        'sigma_bg_Y1': sigma_bg_new,
        'mu_xy_Y1': mu_xy_new,
        'sigma_xy_Y1': sigma_xy_new,
        'mu_I_Y1': mu_I_new,
        'sigma_I_Y1': sigma_I_new,
        'mu_P_Y1': mu_bg_new,
        'sigma_P_Y1': sigma_bg_new,
        'sigma_u1_Y1': sigma_u1_new,
        'sigma_v1_Y1': sigma_v1_new,
        'sigma_u2_Y1': sigma_u2_new,
        'sigma_v2_Y1': sigma_v2_new,
        
        'mu_bg_Y2': mu_bg_new,
        'sigma_bg_Y2': sigma_bg_new,
        'mu_xy_Y2': mu_xy_new,
        'sigma_xy_Y2': sigma_xy_new,
        'mu_I_Y2': mu_I_new,
        'sigma_I_Y2': sigma_I_new,
        'mu_P_Y2': mu_I_new,
        'sigma_P_Y2': sigma_I_new,
        'sigma_u1_Y2': sigma_u1_new,
        'sigma_v1_Y2': sigma_v1_new,
        'sigma_u2_Y2': sigma_u2_new,
        'sigma_v2_Y2': sigma_v2_new,
    ...
    return params_new
def M_step_unlinked(X_data, gammas, Yi, pi):
    gammaY0Z0 = gammas['Y0Z0']
    gammaY0Z1 = gammas['Y0Z1']
    gammaY0Z2 = gammas['Y0Z2']
    gammaY0Z3 = gammas['Y0Z3']
    ...
    effective_gamma_total = gammaY0Z0  + gammaY0Z1  + gammaY0Z2
    sum_gamma = np.sum(effective_gamma_total)
    mu_bg_Y0_new = np.sum(effective_gamma_total * X_data[:,2]) / sum_gamma
    sigma_bg_Y0_new = np.sqrt(np.sum(effective_gamma_total * (X_data[:,2]-mu_bg_Y0_new)**2) / sum_gamma)
    sigma_bg_Y0_new = max(sigma_bg_Y0_new, eps)
   
    effective_gamma_total = gammaY1Z0  + gammaY1Z1  
    sum_gamma = np.sum(effective_gamma_total)
    mu_bg_Y1_new = np.sum(effective_gamma_total * X_data[:,2]) / sum_gamma
    sigma_bg_Y1_new = np.sqrt(np.sum(effective_gamma_total * (X_data[:,2]-mu_bg_Y1_new)**2) / sum_gamma)
    sigma_bg_Y1_new = max(sigma_bg_Y1_new, eps)
    ...
    mu_xy_Y0_new = np.sum(gammaY0Z1[:, np.newaxis] * X_data[:, :2], axis=0) / np.sum(gammaY0Z1)
    var_x = np.sum(gammaY0Z1 * (X_data[:,0]-mu_xy_Y0_new[0])**2) / np.sum(gammaY0Z1)
    var_y = np.sum(gammaY0Z1 * (X_data[:,1]-mu_xy_Y0_new[1])**2) / np.sum(gammaY0Z1)
    sigma_xy_Y0_new = np.sqrt((var_x+var_y)/2)
    sigma_xy_Y0_new = max(sigma_xy_Y0_new, eps)
    
    mu_xy_Y1_new = np.sum(gammaY1Z1[:, np.newaxis] * X_data[:, :2], axis=0) / np.sum(gammaY1Z1)
    var_x = np.sum(gammaY1Z1 * (X_data[:,0]-mu_xy_Y1_new[0])**2) / np.sum(gammaY1Z1)
    var_y = np.sum(gammaY1Z1 * (X_data[:,1]-mu_xy_Y1_new[1])**2) / np.sum(gammaY1Z1)
    sigma_xy_Y1_new = np.sqrt((var_x+var_y)/2)
    sigma_xy_Y1_new = max(sigma_xy_Y1_new, eps)
    
    mu_xy_Y2_new = np.sum(gammaY2Z1[:, np.newaxis] * X_data[:, :2], axis=0) / np.sum(gammaY2Z1)
    var_x = np.sum(gammaY0Z1 * (X_data[:,0]-mu_xy_Y2_new[0])**2) / np.sum(gammaY2Z1)
    var_y = np.sum(gammaY0Z1 * (X_data[:,1]-mu_xy_Y2_new[1])**2) / np.sum(gammaY2Z1)
    ...
    gamma_I = gammaY1Z1 #+ gammaY2Z1 + gammaY2Z2 + gammaY2Z3
    sum_gamma_I = np.sum(gamma_I)
    mu_I_Y1_new = np.sum(gamma_I * X_data[:,2]) / sum_gamma_I #+ 1 * sigma_bg_new
    #mu_I_new = max(mu_I_new,(mu_bg_new + 1 * sigma_bg_new))
    sigma_I_Y1_new = np.sqrt(np.sum(gamma_I * (X_data[:,2]-mu_I_Y1_new)**2) / sum_gamma_I)
    sigma_I_Y1_new = max(sigma_I_Y1_new, eps)
    ...
    sigma_v1_Y0_new = np.sqrt(np.sum(gammaY0Z2 * (v_vals)**2) / np.sum(gammaY0Z2))
    sigma_v1_Y0_new = max(sigma_v1_Y0_new, eps)
    sigma_u1_Y0_new = sigma_u1  # fixed
    
    sigma_v1_Y1_new = np.sqrt(np.sum(gammaY1Z2 * (v_vals)**2) / np.sum(gammaY1Z2))
    sigma_v1_Y1_new = max(sigma_v1_Y1_new, eps)
    sigma_u1_Y1_new = sigma_u1  # fixed
    ...
    sigma_v2_Y0_new = np.sqrt(np.sum(gammaY0Z3 * (v_vals)**2) / np.sum(gammaY0Z3))
    sigma_v2_Y0_new = max(sigma_v2_Y0_new, eps)
    sigma_u2_Y0_new = sigma_u2  # fixed
    
    sigma_v2_Y1_new = np.sqrt(np.sum(gammaY1Z3 * (v_vals)**2) / np.sum(gammaY1Z3))
    sigma_v2_Y1_new = max(sigma_v2_Y1_new, eps)
    sigma_u2_Y1_new = sigma_u2  # fixed
    
    sigma_v2_Y2_new = np.sqrt(np.sum(gammaY2Z3 * (v_vals)**2) / np.sum(gammaY2Z3))
    sigma_v2_Y2_new = max(sigma_v2_Y2_new, eps)
    sigma_u2_Y2_new = sigma_u2  # fixed
    
    params_new = {
        'mu_bg_Y0': mu_bg_Y0_new,
        'sigma_bg_Y0': sigma_bg_Y0_new,
        'mu_xy_Y0': mu_xy_Y0_new,
        'sigma_xy_Y0': sigma_xy_Y0_new,
        'mu_I_Y0': mu_bg_Y0_new,
        'sigma_I_Y0': sigma_bg_Y0_new,
        'mu_P_Y0': mu_bg_Y0_new,
        'sigma_P_Y0': sigma_bg_Y0_new,
        'sigma_u1_Y0': sigma_u1_Y0_new,
        'sigma_v1_Y0': sigma_v1_Y0_new,
        'sigma_u2_Y0': sigma_u2_Y0_new,
        'sigma_v2_Y0': sigma_v2_Y0_new,
        
        'mu_bg_Y1': mu_bg_Y1_new,
        'sigma_bg_Y1': sigma_bg_Y1_new,
        'mu_xy_Y1': mu_xy_Y1_new,
        'sigma_xy_Y1': sigma_xy_Y1_new,
        'mu_I_Y1': mu_I_Y1_new,
        'sigma_I_Y1': sigma_I_Y1_new,
        'mu_P_Y1': mu_bg_Y1_new,
        'sigma_P_Y1': sigma_bg_Y1_new,
        'sigma_u1_Y1': sigma_u1_Y1_new,
        'sigma_v1_Y1': sigma_v1_Y1_new,
        'sigma_u2_Y1': sigma_u2_Y1_new,
        'sigma_v2_Y1': sigma_v2_Y1_new,
        ...
        'mu_I_Y2': mu_I_Y2_new,
        'sigma_I_Y2': sigma_I_Y2_new,
        'mu_P_Y2': mu_I_Y2_new,
        'sigma_P_Y2': sigma_I_Y2_new,
...
for iteration in range(max_iter):
    ...
    #------    
    # Linked parameters 
    gammas_linked = E_step(X_data, A, params_linked, Yi, pi)
    gammas_unlinked = E_step(X_data, A, params_unlinked, Yi, pi)
    
    #-----
    # M-step:
    #-----
    params_linked_new = M_step_linked(X_data, gammas_linked, Yi, pi)
    params_unlinked_new = M_step_unlinked(X_data, gammas_unlinked, Yi, pi)
    
     
    
    #Convergence check.
    if check_convergence(params_linked, params_linked_new, tol) & check_convergence(params_unlinked, params_unlinked_new, tol):
        print(f"Convergence reached at iteration {iteration}")
        break
    
    params_linked = params_linked_new
...
#print("Estimated background intensity: mu =", mu_bg_new, ", sigma =", sigma_bg_new)
print("Estimated background intensity (Y = 2, linked): mu =", params_linked['mu_bg_Y2'], ", sigma =", params_linked['sigma_bg_Y2'])
print("Estimated background intensity (Y = 1, linked): mu =", params_linked['mu_bg_Y1'], ", sigma =", params_linked['sigma_bg_Y1'])
print("Estimated background intensity (Y = 0, linked): mu =", params_linked['mu_bg_Y0'], ", sigma =", params_linked['sigma_bg_Y0'])
print("Estimated background intensity (Y = 2, unlinked): mu =", params_unlinked['mu_bg_Y2'], ", sigma =", params_unlinked['sigma_bg_Y2'])
print("Estimated background intensity (Y = 1, unlinked): mu =", params_unlinked['mu_bg_Y1'], ", sigma =", params_unlinked['sigma_bg_Y1'])
print("Estimated background intensity (Y = 0, unlinked): mu =", params_unlinked['mu_bg_Y0'], ", sigma =", params_unlinked['sigma_bg_Y0'])

# print("Estimated nucleus center (row, col):", mu_xy_nuc)
# print("Estimated nucleus sigma_xy:", sigma_xy)

#print("Estimated nucleus intensity: mu =", mu_I_new, ", sigma =", sigma_I_new)
print("Estimated nucleus intensity (Y = 2, linked): mu =", params_linked['mu_I_Y2'], ", sigma =", params_linked['sigma_I_Y2'])
print("Estimated nucleus intensity (Y = 1, linked): mu =", params_linked['mu_I_Y1'], ", sigma =", params_linked['sigma_I_Y1'])
print("Estimated nucleus intensity (Y = 0, linked): mu =", params_linked['mu_I_Y0'], ", sigma =", params_linked['sigma_I_Y0'])
print("Estimated nucleus intensity (Y = 2, unlinked): mu =", params_unlinked['mu_I_Y2'], ", sigma =", params_unlinked['sigma_I_Y2'])
print("Estimated nucleus intensity (Y = 1, unlinked): mu =", params_unlinked['mu_I_Y1'], ", sigma =", params_unlinked['sigma_I_Y1'])
print("Estimated nucleus intensity (Y = 0, unlinked): mu =", params_unlinked['mu_I_Y0'], ", sigma =", params_unlinked['sigma_I_Y0'])
...
LLY0 = np.sum(np.log(pi))+np.sum(np.log(gammas_unlinked['Y0Z0']))+np.sum(np.log(gammas_unlinked['Y0Z1']))+np.sum(np.log(gammas_unlinked['Y0Z2']))+np.sum(np.log(gammas_unlinked['Y0Z3']))
LLY1 = np.sum(np.log(pi))+np.sum(np.log(gammas_unlinked['Y1Z0']))+np.sum(np.log(gammas_unlinked['Y1Z1']))+np.sum(np.log(gammas_unlinked['Y1Z2']))+np.sum(np.log(gammas_unlinked['Y1Z3']))
LLY2 = np.sum(np.log(pi))+np.sum(np.log(gammas_unlinked['Y2Z0']))+np.sum(np.log(gammas_unlinked['Y2Z1']))+np.sum(np.log(gammas_unlinked['Y2Z2']))+np.sum(np.log(gammas_unlinked['Y2Z3']))
print("Log likelihoods (unlinked):")
print("Y = 0:", LLY0/A)
print("Y = 1:", LLY1/A)
print("Y = 2:", LLY2/A)
print('gammaY0Z2:', np.sum(np.log(gammas_unlinked['Y0Z2']))/A)
print('gammaY1Z2:', np.sum(np.log(gammas_unlinked['Y1Z2']))/A)
print('gammaY2Z2:', np.sum(np.log(gammas_unlinked['Y2Z2']))/A)
print('gammaY0Z3:', np.sum(np.log(gammas_unlinked['Y0Z3']))/A)
print('gammaY1Z3:', np.sum(np.log(gammas_unlinked['Y1Z3']))/A)
print('gammaY2Z3:', np.sum(np.log(gammas_unlinked['Y2Z3']))/A)
LLY0 = np.sum(np.log(pi))+np.sum(np.log(gammas_linked['Y0Z0']))+np.sum(np.log(gammas_linked['Y0Z1']))+np.sum(np.log(gammas_linked['Y0Z2']))+np.sum(np.log(gammas_linked['Y0Z3']))
LLY1 = np.sum(np.log(pi))+np.sum(np.log(gammas_linked['Y1Z0']))+np.sum(np.log(gammas_linked['Y1Z1']))+np.sum(np.log(gammas_linked['Y1Z2']))+np.sum(np.log(gammas_linked['Y1Z3']))
LLY2 = np.sum(np.log(pi))+np.sum(np.log(gammas_linked['Y2Z0']))+np.sum(np.log(gammas_linked['Y2Z1']))+np.sum(np.log(gammas_linked['Y2Z2']))+np.sum(np.log(gammas_linked['Y2Z3']))
print("Log likelihoods (linked):")
print("Y = 0:", LLY0/A)
print("Y = 1:", LLY1/A)
print("Y = 2:", LLY2/A)
print('gammaY0Z2:', np.sum(np.log(gammas_linked['Y0Z2']))/A)
print('gammaY1Z2:', np.sum(np.log(gammas_linked['Y1Z2']))/A)
print('gammaY2Z2:', np.sum(np.log(gammas_linked['Y2Z2']))/A)
print('gammaY0Z3:', np.sum(np.log(gammas_linked['Y0Z3']))/A)
print('gammaY1Z3:', np.sum(np.log(gammas_linked['Y1Z3']))/A)
print('gammaY2Z3:', np.sum(np.log(gammas_linked['Y2Z3']))/A)
#%
...
def plot_mask(gammas):
    ...
    gamma_pol1_img[y_idx, x_idx] = gammaY2Z2  # responsibilities for polarity site 1
    gamma_pol2_img[y_idx, x_idx] = gammaY2Z3  # responsibilities for polarity site 2
    
    # Visualization
    plt.figure(figsize=(8,5))
    
    plt.subplot(3,4,1)
    plt.imshow(gamma_pol1_img, cmap='jet', origin='upper', vmin=0, vmax=1)
    plt.title("GammaY2Z2")
    plt.colorbar(label="Probability")
    
    plt.subplot(3,4,2)
    plt.imshow(gamma_pol2_img, cmap='jet', origin='upper', vmin=0, vmax=1)
    plt.title("GammaY2Z3")
    plt.colorbar(label="Probability")
    
    # Create empty images for the responsibilities.
    gamma_cy_img = np.zeros_like(cropped_cell_mask, dtype=float)
    gamma_nu_img = np.zeros_like(cropped_cell_mask, dtype=float)
    
    gamma_cy_img[y_idx, x_idx] = gammaY2Z0
    gamma_nu_img[y_idx, x_idx] = gammaY2Z1
    
    plt.subplot(3,4,3)
    plt.imshow(gamma_cy_img, cmap='jet', origin='upper', vmin=0, vmax=1)
    plt.title("GammaY2Z0")
    plt.colorbar(label="Probability")
    
    plt.subplot(3,4,4)
    plt.imshow(gamma_nu_img, cmap='jet', origin='upper', vmin=0, vmax=1)
    plt.title("GammaY2Z1")
    plt.colorbar(label="Probability")
    
    plt.tight_layout()
    
    # Next set of images for polarity pitches (gamma Y1)
    gamma_pol1_img = np.zeros_like(cropped_cell_mask, dtype=float)
    gamma_pol2_img = np.zeros_like(cropped_cell_mask, dtype=float)
    
    gamma_pol1_img[y_idx, x_idx] = gammaY1Z2
    gamma_pol2_img[y_idx, x_idx] = gammaY1Z3
    ...
    gamma_cy_img[y_idx, x_idx] = gammaY1Z0
    gamma_nu_img[y_idx, x_idx] = gammaY1Z1
    ...
    # Last set of images for polarity pitches (gamma Y0)
    gamma_pol1_img = np.zeros_like(cropped_cell_mask, dtype=float)
    gamma_pol2_img = np.zeros_like(cropped_cell_mask, dtype=float)
    
    gamma_pol1_img[y_idx, x_idx] = gammaY0Z2
    gamma_pol2_img[y_idx, x_idx] = gammaY0Z3
    ...
    gamma_cy_img[y_idx, x_idx] = gammaY0Z0
    gamma_nu_img[y_idx, x_idx] = gammaY0Z1
...
gamma_pol1_img[y_idx, x_idx] = gammas_unlinked['Y2Z2']  * (cropped_img[y_idx, x_idx] - mu_bg) / sigma_bg #+ gammaY1Z2 * (mu_I - cropped_img[y_idx, x_idx]) / sigma_I)/2# * (cropped_img[y_idx, x_idx] - mu_I) / sigma_I_pol1 # / (gammaY2Z0)  # from EM algorithm
gamma_pol2_img[y_idx, x_idx] = gammas_unlinked['Y2Z3']  * (cropped_img[y_idx, x_idx] - mu_bg) / sigma_bg #+ gammaY1Z3 * (mu_I - cropped_img[y_idx, x_idx]) / sigma_I)/2# * (cropped_img[y_idx, x_idx] - mu_I) / sigma_I_pol2# / (gammaY2Z0) # from EM algorithm
...
for point in contour_cropped:
    y, x = point
    avg_gamma1 = average_intensity_at_point(gamma_pol1_img, cropped_cell_mask, y, x, window_size)
    avg_gamma2 = average_intensity_at_point(gamma_pol2_img, cropped_cell_mask, y, x, window_size)
    avg_gamma0 = average_intensity_at_point(gamma_cy_img, cropped_cell_mask, y, x, window_size)
...
plt.show()
# # Plot the histogram
# plt.hist(X_data[:,2], bins=25, edgecolor='black')  # 'bins' determines the number of bins (bars)
