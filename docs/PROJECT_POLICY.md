...
| `SingleCellQuantificationHPC/quantify_model_based_dense.py` | Runs the stage-4 GFP path over a **stage-3 dense mask series** instead of a re-tracked one. Calls `quantify_one_object` with the same arguments and the same carried `ep_refs` as `one_cell_quantification_1CH.py`, so the numbers are directly comparable; adds the stage-3 provenance columns (branch, GOOD flag, bend angle, expected and achieved span, division frame, `model_only`). Scratch-only output. |
| `bf_pattern.py::bf_pattern_only` | The brightfield path: septum pattern recognition without the GFP mixture fit. |
| `SingleCellQuantificationHPC/build_features_m160.py` | **Stage 5, feature extraction.** Takes stage-4 tables, keeps primary-object rows, cytoplasm-corrects the polarity and septum intensities, places each film of a sequence on a continuous time axis, then runs `signal_analysis.quantify_all_cells`, `signal_cor.quantify_all_cells_acor` and `PCA_utils.load_experiment_features` to produce the eleven-column per-cell feature row. Keyed on `global_cell_id` (P12). **Not** `clustering.cluster_cells_by_amplitude_and_delay`: that is a clustering routine which emits nine weight-normalised columns and no `v` term, and picking it by resemblance rather than by what `FC_AE_data_loader` imports is the mistake P15 exists to prevent. |
